...
@supplierDes_route.route('/getInvoice')
class getInvoice(MethodView):
    ...
    def get(self):
        ...
        invoice = Invoice.query.join(Supplier, Invoice.supplier_id == Supplier.id).filter(Invoice.supplier_id == data).all()
        all_invoices = []
        for i in invoice:
            all_invoices.append({
                'id': i.id,
...
@supplierDes_route.route('/downInvoice')
